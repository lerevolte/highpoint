...
@section('content')
    <h1 class="text-3xl font-bold text-gray-800 dark:text-white mb-6">Добавить подрядчика</h1>
    ...
    <div class="max-w-2xl bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
        <form method="POST" action="{{ route('contractors.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="channel" id="channel" value="{{ old('channel') }}" required placeholder="Яндекс, Meta и т.д."
                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white">
            </div>

            <div>
                <label for="budget" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Бюджет, ₽</label>
                <input type="number" name="budget" id="budget" step="0.01" value="{{ old('budget') }}" required
                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white">
            </div>
